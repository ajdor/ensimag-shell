...
class Test2InOut < Test::Unit::TestCase
  test_order=:defined
  ...
  def test_inout
    ...
    assert_not_nil(a, "sortie incohérente pour ls")
    @pty_write.puts("ls -l totoExpect.txt")
    a = @pty_read.expect(/-r.*totoExpect.txt/, DELAI)
    assert_not_nil(a, "sortie incohérente pour ls -l")
    @pty_write.puts("wc -l totoExpect.txt")
    a = @pty_read.expect(/^(\d+) .*\r\n/, DELAI)
    assert_not_nil(a, "sortie incohérente pour wc -l")
    nbfichier = a[1].to_i
    @pty_write.puts("wc -l < totoExpect.txt")
    a = @pty_read.expect(/^(\d+)\r\n/, DELAI)
    assert_not_nil(a, "sortie incohérente pour wc -l")
    ...
    def test_pipe
      ...
      a = @pty_read.expect(/^(\d+)\r\n/, DELAI)
      assert_not_nil(a, "sortie incohérente pour ls | wc -l")
    end
    ...
    def test_inoutpipe
      ...
      a = @pty_read.expect(/^(\d+)\r\n\1\r\n/, DELAI)
      assert_not_nil(a, "cat titiExpect.txt titiExpect.txt n'a pas rendu deux fois le même entier à la suite")
      nbfichierpipe= a[1].to_i
      @pty_write.puts("wc -l totoExpect.txt")
      a = @pty_read.expect(/^(\d+) totoExpect.txt\r\n/, DELAI)
      assert_not_nil(a, "wc -l totoExpect.txt n'a pas rendu un entier")
      nbfichier= a[1].to_i
